...
export default function EditRoomTypePage({ params }) {
    ...
    useEffect(() => {
        if (!param) return;
        client.get(`room-type/${param}`)
            .then((res) => {
                const room = res.data.rooms;
                if (room) {
                    setFormData({ name: room.name || "", slug: room.slug || "" });
                } else {
                    toast.error("Room type not found");
                }
            })
            .catch(() => toast.error("Failed to load room type"))
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-[#f7f8fd]">
                <p className="text-sm text-[#7a84a6]">Loading...</p>
            </div>
        );
    }
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-5">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <span className="text-[11px] text-[#7a84a6]">
                            URL friendly room type identifier
                        </span>
                    </div>

                    {/* Buttons */}
                    <div className="flex items-center justify-end gap-3 pt-2">
                        <Link href="/admin/room-type">
                            <button
                                type="button"
